...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
       ...
       * becomes knowable.
       *
       * `source` arbitrates between the two seats that know a session. The dock
       * tab's `scope` is the better answer when it has one; the header seat is the
       * fallback, because it is mounted in every shell — including the one where
       * better-sidebar renders the panel and this seat renders nothing at all.
       *
      ...
      function adoptSession(id, source) {
        ...
        if (sessionSource === 'dock' && source !== 'dock') return
        if (next !== sessionId) {
          sessionId = next
    ...
    function pickScheme(schemes, projectName) {
      ...
      function Toggle(props) {
        ...
        // the shared state. With better-sidebar installed that gate closed for
        // good, which left the dock tab's `scope.sessionId` as the only way to
        // name a session — and an unscoped tab supplies none. The panel then sent
        // every read unnamed and the host, asked to guess, answered with its own
        // launch directory. The arbitration in `adoptSession` keeps the dock
